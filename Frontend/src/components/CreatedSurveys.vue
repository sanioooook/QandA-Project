<template>
  <div id="CreatedSurveys">
    <h1>Все опросы</h1>
    <div v-for="(survey) in surveys" :key="survey.index">
      <div class="Survey" :data-title="survey.god.login">
      <p>{{survey.question}}</p>
        <div class="Answer" v-for="(answer) in survey.answers" :key="answer.indextwo" ><!--:data-title="survey.voters" -->
    ...
    Vote (survey, answer) {
      if (answer.vote) {
        survey.loginvote = true
          this.$store.dispatch('Vote', {voter: this.$store.getters.getUser, idAnswer: answer.id })
          .then(data => {
            console.log(data)
            this.start()
          })
...
.Survey:hover::after {
  content: "Created survey: " attr(data-title);
    ...
    right: 30%;/*  top: 30%; Положение подсказки */
