<template>
  <div id="UserSurveys">
    <h1>Все созданные вами опросы</h1>
    <div v-for="(survey) in surveys" :key="survey.index">
      <div v-if="survey.god.login == $store.getters.getUser.login" class="Survey" :data-title="survey.god.login">
      <p>{{survey.question}}</p>
      ...
      <div>Creator survey: {{survey.god.login}}</div>
      </div>
    </div>
  </div>
</template>
